...
- **タブナビゲーション**:
  ...
  - 会員情報タブ
- **予約履歴表示**:
  - 全予約の一覧表示（予約確認済み、利用済み、キャンセル済み）
  - 予約番号、予約日、ステータス
  - 往路・復路のフライト情報（便名、区間、日時、座席クラス）
  - 乗客数、合計金額
  - クリックで詳細表示
- **予約詳細モーダル**:
  - 完全なフライト情報（便名、航空会社、座席番号、所要時間）
  - 乗客情報（氏名、性別、生年月日、国籍、パスポート番号）
  - 連絡先情報
  - 料金詳細（基本料金、座席アップグレード、税金・手数料、合計）
  - ステータスバッジ（予約確認済み、利用済み、キャンセル済み）
- **予約キャンセル機能**:
  - 予約詳細画面からキャンセルボタン
  - キャンセル確認モーダル
  - ステータス更新とリスト再読み込み
